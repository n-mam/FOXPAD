...
function index(jb)
{
  ...
  switch (jb.api.toUpperCase())
  {
    ...
    case 'SIGNUP':
    {
      ...
      let sql = `select id, password from User where name = '${jb.name}'`;
    ...
    case "TRAIL":
    {
      ...
      if (u.isDefined(jb.points))
      {
        let age = 0;
        let gender = 'N/A';

        if (u.isDefined(jb.demography) && jb.demography.length)
        {
          let s = jb.demography.substring(jb.demography.length - 4);
          let arr = s.split(" ");
          age = parseInt(arr[0]);
          gender = arr[1] === "F" ? "Female" : "Male";
        }

        sql = `insert into trails (cid, aid, uid, ts, path, demography, thumbnail, age, gender)
               values
               (${db.esc(jb.cid)},
                ${db.esc(jb.aid)},
                ${db.esc(jb.uid)},
                NOW(),
                ST_GeomFromText('MULTIPOINT(${jb.points})'),
                ${db.esc(jb.demography)},
                ${db.esc(jb.thumbnail)},
                ${db.esc(age)},
                ${db.esc(gender)})`;
      }
      else if (u.isDefined(jb.label))
      {
        sql = `insert into trails (cid, aid, uid, ts, label, count)
               values
               (${db.esc(jb.cid)},
                ${db.esc(jb.aid)},
                ${db.esc(jb.uid)},
